---
name: update-plugin
description: Update the installed elite-next plugin to the latest published version from the elitebusinesssolutions — use when the user asks to update the plugin, get the latest skills/hooks, or check for plugin updates.
---

Update the `elite-next` plugin (this plugin, once installed in a consumer project) to the newest version published on the `elitebusinesssolutions` marketplace.

## Step 1 — Check whether autoUpdate is already handling this

Look for an `extraKnownMarketplaces.elitebusinesssolutions.autoUpdate` field set to `true` in the project's `.claude/settings.json` (this is what elite-next's own README's "Consumer project setup" section recommends). If it's already `true`:
...
If `autoUpdate` isn't set — or there's no `extraKnownMarketplaces` entry for `elitebusinesssolutions` at all — continue with the manual steps, and see Step 3 for adding it going forward.
...
> Either the marketplace catalog is stale (re-run `claude plugin marketplace update elitebusinesssolutions`) or no version bump has been merged to `main` yet. Check `.claude-plugin/plugin.json`'s `version` field directly on the `main` branch of the repo.

**`claude plugin update` fails with "plugin not found":**

> The marketplace isn't registered locally yet. Run `claude plugin marketplace add elitebusinesssolutions/claude-dev-plugin` first, then retry.
